...
export function createCampusFloorLogo(T){
  ...
  function paint(){
    ...
    const icon=220,gap=48,total=icon+gap+fomoWidth+gap+campusWidth,start=(canvas.width-total)/2;
    ...
    ctx.fillStyle='#6b6f7b';ctx.font='600 150px Aeonik, Arial, sans-serif';ctx.fillText('/campus',start+icon+gap+fomoWidth+gap,209);
  }
  paint();
  const map=new T.CanvasTexture(canvas);map.colorSpace=T.SRGBColorSpace;map.anisotropy=8;
  const mesh=new T.Mesh(new T.PlaneGeometry(10,10*320/1536),new T.MeshBasicMaterial({map,toneMapped:false}));
  mesh.name='fomo-campus-floor-logo';mesh.position.set(0,.14,34);mesh.rotation.x=-Math.PI/2;
