...
export function createCampusBannerTexture(T){
  ...
  function paint(){
    ...
    ctx.save();ctx.translate(start,50);ctx.scale(icon/100,icon/100);ctx.fillStyle='#f7f7f7';for(const path of FOMO_MARK_PATHS)ctx.fill(new Path2D(path));ctx.restore();
...
export function createVehicleLogoTexture(T){
  ...
  function paint(){
    ...
    ctx.save();ctx.translate(22,14);ctx.scale(1.95,1.95);ctx.fillStyle='#ffffff';for(const path of FOMO_MARK_PATHS)ctx.fill(new Path2D(path));ctx.restore();
