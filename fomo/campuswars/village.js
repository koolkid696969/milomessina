...
import {movePlayer,walkRoute} from './village-layout.js';
...
function startVillage(){
  ...
  function setMode(next){
    const previous=mode;mode=next;held.clear();destination=null;route=[];destinationMarker.visible=false;player.visible=next==='walk';shell.classList.toggle('walking',next==='walk');
    walkButton.setAttribute('aria-pressed',String(next==='walk'));overviewButton.setAttribute('aria-pressed',String(next==='overview'));
    if(next==='walk'){
      if(previous==='overview'&&Math.hypot(player.position.x-wantedTarget.x,player.position.z-wantedTarget.z)>45){
        for(const [dx,dz] of [[0,0],[4,0],[-4,0],[0,8],[0,-8],[8,8],[-8,-8]])if(isWalkable(wantedTarget.x+dx,wantedTarget.z+dz)){player.position.set(wantedTarget.x+dx,.2,wantedTarget.z+dz);break;}
      }
      wantedRadius=13;wantedPhi=.43;wantedTheta=.3;wantedTarget.copy(player.position).add(new THREE.Vector3(0,1,0));status.textContent='YOU’RE ON THE ROW';help.textContent='WASD or arrows to walk · drag to look · tap a house';}
    else{wantedTarget.set(0,0,0);wantedRadius=overviewRadius();wantedPhi=.79;wantedTheta=.68;status.textContent='THE WHOLE VILLAGE';help.textContent=viewport.clientWidth<650?'Walk around to explore · tap a place or house':'Drag to orbit · shift-drag to explore · tap a house';}
    wake();
  }
  function choose(id,focus=false){
    ...
    if(focus){
      if(mode==='walk'){goTo({x:Math.sign(anchor.lot.x)*11,z:anchor.lot.z});}
  ...
  function goTo(point){
    route=walkRoute(player.position,point);destination=route.shift()||null;destinationMarker.position.set(point.x,.21,point.z);destinationMarker.visible=Boolean(destination);
    if(!destination)status.textContent='Tap an open path nearby to walk there.';
    wake();
  }
  document.querySelectorAll('[data-place]').forEach(button=>button.addEventListener('click',()=>{
    const point={x:Number(button.dataset.x),z:Number(button.dataset.z)};
    if(mode==='walk')goTo(point);else{wantedTarget.set(point.x,1,point.z);wantedRadius=viewport.clientWidth<650?45:36;wantedPhi=.63;}
    status.textContent=button.textContent.toUpperCase();wake();
  }));
  ...
  canvas.addEventListener('pointerup',e=>{
    ...
    if(mode==='walk'){const ground=raycaster.intersectObject(village.ground)[0];if(ground){goTo({x:ground.point.x,z:ground.point.z});}}
  });
  ...
  canvas.addEventListener('keydown',e=>{if(e.code==='Escape'){setMode('overview');return;}if(mode!=='walk'||!keys[e.code])return;e.preventDefault();held.add(keys[e.code]);destination=null;route=[];destinationMarker.visible=false;wake();});
  ...
  function frame(now){
    ...
    if(mode==='walk'){
      ...
      else if(destination){dx=destination.x-player.position.x;dz=destination.z-player.position.z;const d=Math.hypot(dx,dz);if(d<.22){destination=route.shift()||null;destinationMarker.visible=Boolean(destination);}else{dx/=d;dz/=d;moving=true;}}
      ...
      const dot=document.getElementById('village-map-player');dot.style.left=`${Math.max(5,Math.min(95,47+player.position.x*1.15))}%`;dot.style.top=`${Math.max(5,Math.min(85,47+player.position.z*1.26))}%`;
    }
    ...
    if(mode==='walk'&&!isWalkable(camera.position.x,camera.position.z)){
