...
The town blocks’ former shops are now limestone and glass trading halls with monitor islands, a scrolling façade ticker, three candlestick sculptures and a SOL/USD chart board. **Exchange** jumps to the northeast market and opens a small accessible price/status panel with a link to [fomo.family](https://fomo.family/). The original building footprint is preserved. Screens remain outside the static geometry batches and release their textures when a district streams out. Market data survives chunk replacement and longer chapter rows; the ticker follows the existing activity clock, including pause and reduced motion.

`market-feed.js` reads only `/api/campus-market`. That Vercel function calls fomo’s own `https://prod-api.fomo.family/proxy/getBars` endpoint for SOL/USD hourly candles and returns an allowlisted snapshot. There is no alternate market provider. The ticker shows the latest candle close and change since the first candle’s open; the chart preserves actual timestamp gaps across up to 24 hourly candles. The current candle may still be forming. A failed refresh retains the last successful fomo data with a delayed label. Missing or rejected authorization clears quotes and displays “Fomo connection required.”

**Live activation is blocked on authorized fomo access.** The currently served [fomo application](https://fomo.family/) requires a bearer access token for chart requests. Its public price URLs now load the application shell rather than exposing a public price feed. Configure `FOMO_MARKET_ACCESS_TOKEN` on the Vercel server only if fomo supplies approved access for this integration, then deploy. Do not put a visitor’s session token in this variable or ship any credential to the browser. No credential is currently configured by this change, and no authenticated live response has been verified. A normal user sign-in on fomo.family does not automatically connect this separate website.

The browser refreshes once per minute while visible; the server coalesces concurrent reads and caches successful results for 60 seconds. No upstream error body, private account data or credential is returned. [tests/market-gallery.html](tests/market-gallery.html) shows the exchange and its current connection state. A static-only local server does not run the API function and will show “Fomo feed unavailable.” Tests cover the actual fomo response schema, authorization failures, source validation, stale behavior, polling, screen disposal and the Exchange destination.
...
House exteriors use red and brown brick, warm ivory, slate blue, sage, sandstone and other subdued traditional finishes. Each chapter is allocated a different color, independent of architectural style and ranking. Existing colors persist during live updates, and additional chapters receive unused finishes or natural shade variations. Street-view road markers are simple thin circles with no directional arrows; looking around uses dragging or the left/right keys.
