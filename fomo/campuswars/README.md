...
On page load, `village-intro.js` supplies a five-second camera route and three synchronized captions: the chapter trading competition, building a house with 15 members / 80% onboarding qualification, and the $500,000 commitment. The tour descends across the row and ends at the existing composition, approximately `(10.194, 12.378, -52.779)`, looking at `(1.808, 2, -8.101)`. Captions and their shading fade out over the final 350ms. Its clock uses visible elapsed time so slow frame rates do not extend the introduction; hidden tabs and offscreen time pause it. Reduced motion shows the captions with a stationary camera. Skip returns to the village view; direct camera interaction clears the intro immediately. About includes a replay button.
...
All 49 automated checks pass, including camera/caption timing, skip, replay, interruption, reduced motion, slow frames and pausing the tour offscreen, plus the existing live registration, competition, scene, crowd and motion tests. Desktop (1280×720) and mobile (390×844) browser review covered the full-screen scene, intro, chapter drawer, chapter selection and About/replay. No browser console errors were observed. The measurements below are historical rendering measurements, not a new benchmark of the full-screen layout.
