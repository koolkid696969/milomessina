...
Choose **Street view** to enter the boulevard at eye level. Click or tap a white road marker to move to that point, drag to look around, and use the forward/back, turn-around and exit controls. Up/down arrow keys step along the street, left/right look around, and Escape returns to the overview. House clicks select the chapter while keeping the street perspective; focusing a chapter from the drawer returns to the orbit view.
