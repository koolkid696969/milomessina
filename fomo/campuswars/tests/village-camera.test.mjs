...
function cameraHarness(reduced=false,initialHash=''){
  ...
  const sandbox={THREE:{...THREE,WebGLRenderer:Renderer},createVillage:()=>({world:new THREE.Group(),anchors:[{id:'sigma-chi-sdsu',lot:{x:-20,z:-19}}],selection:new THREE.Object3D(),competition:{badges:[]},animateCrowd(){},animateEffects(){}}),createDistricts:()=>({root:new THREE.Group(),update(){return false;},animate(){}}),CustomEvent:class{constructor(type,options){this.type=type;this.detail=options.detail;}},document:{getElementById:element,addEventListener(){},dispatchEvent(event){selections.push(event.detail.id);},hidden:false},matchMedia:query=>({matches:query.includes('reduced-motion')&&reduced}),devicePixelRatio:1,location:{hash:initialHash},URLSearchParams,ResizeObserver:class{observe(){}},IntersectionObserver:class{constructor(fn){intersection=fn;}observe(){}},addEventListener(){},requestAnimationFrame:fn=>{frame=fn;return 1;},cancelAnimationFrame(){}};
  const source=fs.readFileSync(new URL('../village.js',import.meta.url),'utf8').replace(/^import .*;$/gm,'');
  vm.runInNewContext(source,sandbox);
  let now=100;
  return {selections,show(visible){intersection([{isIntersecting:visible}]);},step(seconds){for(let t=0;t<seconds;t+=1/60){now+=1000/60;const fn=frame;frame=null;fn?.(now);}return camera.position.clone();},drag(){events.get('canvas:pointerdown')({button:0,pointerId:1,clientX:0,clientY:0});},reset(){events.get('village-overview:click')();}};
}
