...
test('every chapter banner shows actual joined members against the rounded-up 80% target',()=>{
  for(const [id,name] of [['sigma-chi-sdsu','Sigma Chi'],['kappa-sigma-coastal','Kappa Sigma'],['phi-delta-theta-tampa','Phi Delta Theta'],['phi-kappa-psi-vt','Phi Kappa Psi'],['tau-kappa-epsilon-tampa','Tau Kappa Epsilon'],['new-chapter','New Chapter']]){
    for(const [joined,active,target] of [[34,69,56],[49,70,56],[60,100,80],[0,21,17],[90,100,80]]){
      ...
      assert(text.includes(`${joined} / ${target}`),`${id} uses the qualification target`);
    }
  }
});
